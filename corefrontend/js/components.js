...
const Icon = {
  ...
  badge: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>`,
  ...
  phone: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.54 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>`,
  ...
  sparkles: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l1.5 3.5L17 8l-3.5 1.5L12 13l-1.5-3.5L7 8l3.5-1.5L12 3z"/><path d="M19 15l.75 1.75L21.5 17.5l-1.75.75L19 20l-.75-1.75L16.5 17.5l1.75-.75L19 15z"/><path d="M5 17l.5 1.25L6.75 18.75 5.5 19.25 5 20.5l-.5-1.25L3.25 18.75 4.5 18.25 5 17z"/></svg>`,
  ...
  wifi: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12.55a11 11 0 0 1 14.08 0"/><path d="M1.42 9a16 16 0 0 1 21.16 0"/><path d="M8.53 16.11a6 16 0 0 1 6.95 0"/><line x1="12" y1="20" x2="12.01" y2="20"/></svg>`,
  signal: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="2" y1="20" x2="2" y2="20"/><line x1="7" y1="15" x2="7" y2="20"/><line x1="12" y1="10" x2="12" y2="20"/><line x1="17" y1="5" x2="17" y2="20"/><line x1="22" y1="2" x2="22" y2="20"/></svg>`,
  ...
  infinity: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 12c-2-2.5-4-4-6-4a4 4 0 0 0 0 8c2 0 4-1.5 6-4z"/><path d="M12 12c2 2.5 4 4 6 4a4 4 0 0 0 0-8c-2 0-4 1.5-6 4z"/></svg>`,
...
function _rootPath(file) {
  /* Works whether the page is at /index.html or /subdir/page.html */
  const base = document.querySelector('base');
  if (base) return base.href + file;
  /* Walk up from current script location to the root */
...
async function loadHeader(activePage = '') {
  ...
  if (!root) return;
  try {
    const res = await fetch(_rootPath('header.html'));
...
async function loadFooter() {
  ...
  if (!root) return;
  try {
    const res = await fetch(_rootPath('footer.html'));
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const html = await res.text();
    root.innerHTML = html;
  } catch (e) {
...
function initHeader() {
  ...
  const menuSVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="18" x2="21" y2="18"/></svg>`;
  ...
  function toggleMenu() {
    ...
    icon.innerHTML = open ? closeSVG : menuSVG;
...
/* ── STATUS STATE ── */
function statusStateHTML({ title, message, onRetry, onBack }) {
  return `
    <div class="status-state">
      <div class="status-icon">${Icon.alertCircle}</div>
      <p class="status-title">${title}</p>
      <p class="status-msg">${message}</p>
      <div class="status-actions">
        ${onBack ? `<button class="btn-outline" onclick="${onBack}">Go Back</button>` : ''}
        ${onRetry ? `<button class="btn-primary" style="padding:0.5rem 1.5rem" onclick="${onRetry}">Retry</button>` : ''}
      </div>
    </div>
  `;
}
